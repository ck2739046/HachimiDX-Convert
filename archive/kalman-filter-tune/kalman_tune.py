"""
kalman_tune.py — 7 维恒速 Kalman 离线调参

用 detect_result.txt (YOLO 检测) 和 track_result.txt (dump 真值)
对 KalmanBoxTracker 的 Q/R 超参进行贝叶斯优化。

7 维状态: [x,y,s,r, vx,vy,vs]，s=w*h, r=w/h。
...
from src.core.auto_convert.detect.oc_sort import KalmanBoxTracker
...
def _build_kalman_with_params(
    ...
    q_pos: float, q_vel: float, q_s: float,
    r_pos: float,
) -> KalmanBoxTracker:
    """创建 KalmanBoxTracker 并用给定缩放因子覆写 Q/R.
    init_bbox: [x1,y1,x2,y2, cx,cy, conf, cls, idx] from detect array

    7D 状态: [x,y,s,r, vx,vy,vs]
    """
    kf_bbox = np.array([init_bbox[0], init_bbox[1], init_bbox[2], init_bbox[3],
                         init_bbox[6], init_bbox[7], init_bbox[8]], dtype=np.float64)
    kf = KalmanBoxTracker(kf_bbox)

    # Q (7×7): indices 0,1=x,y; 2,3=s,r; 4,5=vx,vy; 6=vs
    kf.kf.Q[0, 0] *= q_pos; kf.kf.Q[1, 1] *= q_pos
    kf.kf.Q[2, 2] *= q_s;   kf.kf.Q[3, 3] *= q_s
    kf.kf.Q[4, 4] *= q_vel; kf.kf.Q[5, 5] *= q_vel
    kf.kf.Q[6, 6] *= q_s
    # 同时也缩放初始 P
    kf.kf.P[0, 0] *= q_pos; kf.kf.P[1, 1] *= q_pos
    kf.kf.P[2, 2] *= q_s;   kf.kf.P[3, 3] *= q_s
    kf.kf.P[4, 4] *= q_vel; kf.kf.P[5, 5] *= q_vel
    kf.kf.P[6, 6] *= q_s

    # R (4×4): 位置观测噪声, s/r 保持默认 (已乘 10)
...
def evaluate_track(
    ...
    kf = _build_kalman_with_params(init_obs, q_pos, q_vel, q_s, r_pos)
...
def evaluate_params(
    ...
    q_pos, q_vel, q_s, r_pos = params
    total_err = 0.0
    total_frames = 0
    for tframes in all_tracks:
        mean_e, _ = evaluate_track(tframes, q_pos, q_vel, q_s, r_pos)
        total_err += mean_e * (len(tframes) - 1)
        total_frames += (len(tframes) - 1)
    mpe = total_err / total_frames if total_frames else float('inf')
    if verbose:
        print(f"  MPE={mpe:.3f}  q=({q_pos:.4f},{q_vel:.4f},{q_s:.4f}) r_pos={r_pos:.4f}")
    return mpe
...
def grid_search(
    ...
    """粗粒度 L1 网格搜索 q_pos/q_vel/q_s（log 间隔）."""
    ...
    for qp in q_candidates:
        for qv in q_candidates:
            for qs in q_candidates:
                mpe = evaluate_params((qp, qv, qs, 1.0), all_tracks)
                i += 1
                if mpe < best_mpe:
                    best_mpe = mpe
                    best_params = (qp, qv, qs)
                if i % 10 == 0:
                    print(f"  [{i}/{total}] best MPE={best_mpe:.3f} @ q=({best_params[0]},{best_params[1]},{best_params[2]})")

    print(f"\nGrid best: MPE={best_mpe:.3f} @ q_pos={best_params[0]}, q_vel={best_params[1]}, q_s={best_params[2]}")
...
def bayesian_optimize(
    ...
    space = [
        ...
        Real(0.005, 200.0, name='q_s', prior='log-uniform'),
    ...
    def objective(**kwargs):
        params = (kwargs['q_pos'], kwargs['q_vel'], kwargs['q_s'], kwargs['r_pos'])
...
def main():
    ...
    (gqp, gqv, gqs), grid_mpe = grid_search(all_tracks)
    ...
    # ── 硬编码到 oc_sort.py 的建议 ──
    print("\n--- 建议写入 KalmanBoxTracker.__init__ 的代码 ---")
    print("    # 过程噪声 Q — tuned on SLIDE tracks (7D CV)")
    print(f"    self.kf.Q[0, 0] *= {bp[0]:.6f}; self.kf.Q[1, 1] *= {bp[0]:.6f}  # q_pos")
    print(f"    self.kf.Q[4, 4] *= {bp[1]:.6f}; self.kf.Q[5, 5] *= {bp[1]:.6f}  # q_vel")
    print(f"    self.kf.Q[2, 2] *= {bp[2]:.6f}; self.kf.Q[3, 3] *= {bp[2]:.6f}  # q_s")
    print(f"    self.kf.Q[6, 6] *= {bp[2]:.6f}")
    print(f"    self.kf.R[0, 0] *= {bp[3]:.6f}; self.kf.R[1, 1] *= {bp[3]:.6f}  # r_pos")
