...
class NoteDetector:
    ...
    def classification_module(self, track_results, std_video_path,
        ...
        try:
            ...
            for frame_number in range(total_frames):
                ...
                # 提取当前帧的所有采样图像
                cropped_images = self._extract_note_images_in_frame(frame, this_frame_sample_plan, frame_number)
                ...
                while len(images_batch_buffer) >= batch_cls:
                    ...
                    last_time, last_counter = self.print_progress('cls', ' Images/s', counter, total_cls_quantity, last_time, last_counter)
    ...
    def _extract_note_images_in_frame(self, frame, this_frame_sample_plan, frame_number):
        try:
            cropped_images = []
    ...
    def _merge_cls_into_track_results(self, track_results, cls_results_all):
        ...
        for cls_result in cls_results_all:
            ...
            specific_class_id = self.get_specific_class_id(class_id, is_break, is_ex)
