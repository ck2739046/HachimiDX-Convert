...
class NoteAnalyzer:
    ...
    # debug
    @log_error
    def preprocess_slide_data(self):
        '''
        收集所有slide音符的数据
        暂时只分离出星星头，视为 tap note 处理, 忽略后续轨迹移动
        过滤轨迹过短的音符
        计算音符方向
        计算音符到圆心的距离
        过滤刚离开起点的和马上要到终点的音符数据 (10%-90%距离)
        ...
        for track_id, track_data in self.track_data.items():
            ...
            # 检查轨迹存在
            if not valid_track_path:
                # print(f"preprocess_slide_data: no valid_track_path for track_id {track_id}")
                continue
            valid_track_path.sort(key=lambda x: x[0]) # 按frame排序
            
            # 检验长度
            if len(valid_track_path) < 6:
                # print(f"preprocess_slide_data: valid_track_path too short for track_id {track_id}, length: {len(valid_track_path)}")
                continue
            # 检验方位一致
            positions = [x[5] for x in valid_track_path]
            if len(set(positions)) != 1:
                # print(f"preprocess_slide_data: positions not consistent for track_id {track_id}")
                continue
            # 检查dist是否递增 (允许微小回退 -0.5* start_tolerance)
            dists = [x[6] for x in valid_track_path]
            if not all(later - earlier > -0.5 * start_tolerance for earlier, later in zip(dists, dists[1:])):
                # print(f"preprocess_slide_data: dist not increasing for track_id {track_id}")
                continue
            # 检查dist是否在头尾 (20%-80%)
            if dists[0] > valid_judgeline_start + start_tolerance or dists[-1] < valid_judgeline_end - end_tolerance:
                # print(f"preprocess_slide_data: dist out of range for track_id {track_id}, start_dist: {dists[0]}, end_dist: {dists[-1]}")
                continue
            # 添加到slide_data
            path = []
            for frame_num, x1, y1, x2, y2, position, dist_to_center in valid_track_path:
        ...
        if not slide_data:
            print("preprocess_slide_data: no slide data")
            return {}
    ...
    def main(self, main_folder: str, bpm: float, chart_lv: int, base_denominator: int):
        try:
            ...
            # 暂时仅处理星星头，视为 tap 处理
            slide_info = {}
            slide_data = self.preprocess_slide_data()
            if slide_data:
